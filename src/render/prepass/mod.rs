...
impl Plugin for PrepassPipelinePlugin {
    fn build(&self, app: &mut App) {
        ...
        render_app
            ...
            .init_gpu_resource::<SpecializedPointCloudPipelines<PrepassPipelineSpecializer>>();
    }
}
...
// type PreviousMeshFilter = With<PointCloudChunk3d>;
// // #[cfg(feature = "meshlet")]
...
///
/// 1. If GLSL support is enabled at the feature level, then in debug mode `naga_oil` will attempt
///    to compile all shader modules under GLSL to check validity of names, even if GLSL isn't
///    actually used. This will cause a crash if binding arrays are enabled, because binding arrays
///    are currently unimplemented in the GLSL backend of Naga. Therefore, we disable binding arrays
///    if the `shader_format_glsl` feature is present.
///
/// 2. If there aren't enough texture bindings available to accommodate all the binding arrays, the
///    driver will panic. So we also bail out if there aren't enough texture bindings available in
///    the fragment shader.
///
/// 3. If binding arrays aren't supported on the hardware, then we obviously can't use them. Adreno
///    <= 610 claims to support bindless, but seems to be too buggy to be usable.
///
/// 4. If binding arrays are supported on the hardware, but they can only be accessed by uniform
///    indices, that's not good enough, and we bail out.
///
...
pub fn skins_use_uniform_buffers(limits: &WgpuLimits) -> bool {
    bevy::render::storage_buffers_are_unsupported(limits)
}
